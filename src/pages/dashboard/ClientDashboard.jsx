        ...
        <div style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border-primary)', borderRadius: 16, padding: 24 }}>
          ...
          {loading ? <Spinner /> : jobs.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>
              <div style={{ fontSize: 36, marginBottom: 8 }}><i className="fi fi-rr-box-open"></i></div>
                ...
                return (
                  <div key={job._id} style={{ 
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >{job.title}</Link>
                      <div style={{ fontSize: 11, color: 'var(--text-faint)', marginTop: 3 }}>
                        {job.category} · {job.budgetType === 'range' && job.budgetMax ? `${formatBDT(job.budget)} - ${formatBDT(job.budgetMax).replace('৳ ', '')}` : formatBDT(job.budget)} · {daysLeft(job.deadline)} · {job.bidsCount || 0} bids
                      </div>
                    </div>
