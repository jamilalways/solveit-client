...
export default function ProblemDetail() {
  ...
  if (loading) {
    ...
    return user ? <DashboardLayout>{loader}</DashboardLayout> : <><Navbar />{loader}</>
  }
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {problem.files.map((f, i) => {
                  ...
                  const fileUrl = getImageUrl(f.url)
                  return (
                    <div key={i}>
                      {isImage ? (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border-primary)', borderRadius: 16, padding: 20 }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 18 }}>
            {problem.budgetType === 'range' ? 'Price range budget' : 'Fixed price budget'}
          </div>
          {[
        ...
        <div style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border-primary)', borderRadius: 16, padding: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            {problem.client?.avatar ? (
              <img 
                src={getImageUrl(problem.client.avatar)} 
        ...
        <form onSubmit={handleSubmitBid}>
          {[
            ...
            <div key={f.name} style={{ marginBottom: 14 }}>
              ...
              <input
                ...
                style={{ width: '100%', border: '1.5px solid var(--input-border)', borderRadius: 10, padding: '10px 13px', fontSize: 14, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box', background: 'var(--input-bg)', color: 'var(--text-primary)' }}
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <textarea
              ...
              style={{ width: '100%', border: '1.5px solid var(--input-border)', borderRadius: 10, padding: '10px 13px', fontSize: 14, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box', resize: 'vertical', background: 'var(--input-bg)', color: 'var(--text-primary)' }}
              onFocus={(e) => (e.target.style.borderColor = 'var(--input-focus)')}
              onBlur={(e)  => (e.target.style.borderColor = 'var(--input-border)')}
            />
          </div>
