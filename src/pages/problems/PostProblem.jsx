...
export default function PostProblem() {
  ...
  const [form, setForm] = useState({
    title: '', category: '', customCategory: '', description: '', budget: '', budgetType: 'fixed', budgetMin: '', budgetMax: '', deadline: '',
  })
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.budgetType === 'range') {
      ...
      if (!min || !max) return setError('Please enter both min and max budget.')
      if (min >= max)  return setError('Max budget must be greater than min budget.')
    }
    ...
    <DashboardLayout>
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 0 40px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ background: 'var(--bg-card)', border: '1.5px solid var(--border-primary)', borderRadius: 20, padding: 28 }}>
          ...
          {/* Budget */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: form.budgetType === 'range' ? 10 : 18 }}>
          ...
          {/* Price range fields — shown only when budgetType === 'range' */}
          {form.budgetType === 'range' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 18 }}>
              <div>
                <label style={labelStyle}>Min Budget (৳) *</label>
                <input name="budgetMin" type="number" required min="1" placeholder="e.g. 500" value={form.budgetMin} onChange={handleChange} style={inputStyle}
                  onFocus={(e) => (e.target.style.borderColor = 'var(--input-focus)')}
                  onBlur={(e)  => (e.target.style.borderColor = 'var(--input-border)')}
                />
              </div>
              <div>
                ...
                <input name="budgetMax" type="number" required min="1" placeholder="e.g. 1000" value={form.budgetMax} onChange={handleChange} style={inputStyle}
                  onFocus={(e) => (e.target.style.borderColor = 'var(--input-focus)')}
                  onBlur={(e)  => (e.target.style.borderColor = 'var(--input-border)')}
                />
              </div>
